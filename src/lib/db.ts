...
export const updateBook = (id: string, updates: { available: boolean; dueDate?: string; borrowedBy?: string | null }) => {
    books = books.map(book => book.id === id ? { ...book, ...updates } : book);
    return getBookById(id);
};
...
// New function to specifically handle borrowing/returning a book
export const updateBookStatus = (
    ...
    // Use the existing updateBook function
    return updateBook(id, updates);
};

// Function to initialize books in localStorage (for client-side persistence)
export const syncBooksToLocalStorage = () => {
    if (typeof window !== 'undefined') {
        localStorage.setItem('library-books', JSON.stringify(books));
    }
};

// Function to load books from localStorage (if available)
export const loadBooksFromLocalStorage = () => {
    if (typeof window !== 'undefined') {
        ...
        if (storedBooks) {
            ...
            // If no books in localStorage, save the current ones
            syncBooksToLocalStorage();
        }
    }
    return books;
};
...
// Initialize localStorage on module load (client-side only)
if (typeof window !== 'undefined') {
    // We use setTimeout to ensure this runs after module initialization
    setTimeout(() => {
